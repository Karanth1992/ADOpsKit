...
.DESCRIPTION
    Discovers all domains in the forest and, for each domain
    controller, collects the following by targeting it directly
    from the machine running this script (Dcdiag's /s: switch and
    WMI's -ComputerName both work remotely on their own):
    ...
    Read-only. Uses WMI/DCOM and Dcdiag.exe remote targeting only -
    no PSRemoting/WinRM is used or required, since WinRM is assumed
    blocked in this environment. Outputs a colour-coded HTML report
    to a central network share and optionally sends it by email.
    Each domain is collected in isolation so one unreachable domain
    does not prevent the report from covering the rest.

.PARAMETER OutputFolder
    Folder where the HTML report file is written.
    Defaults to $env:TEMP.

.EXAMPLE
    Get-ADForestHealth
    Runs the health check and writes the HTML report to $env:TEMP.

.EXAMPLE
    Get-ADForestHealth -OutputFolder "\\ServerName\C$\Scripts\HealthCheck\Reports"
    Runs the health check and writes to a network share.
...
.NOTES
    ...
    Version:  9.0
    Created:  26-02-2026
    Run as Domain Admin or equivalent with WMI/DCOM access to all domain
    controllers. WinRM/PSRemoting is not used.
...
#>
    ...
    # Invoked locally per domain. Every remote touchpoint inside
    # (Dcdiag /s:, Get-WmiObject -ComputerName) targets DCs directly
    # and does not require WinRM/PSRemoting.
    ...
    # Each domain is isolated in its own try/catch so one unreachable
    # domain does not abort collection for the rest of the forest.

    $perDomainResults = @{}
    $failedDomains     = [System.Collections.Generic.List[string]]::new()

    foreach ($domain in $allDomains) {
        Write-Host "Collecting health data from domain '$domain'..."
        try {
            $perDomainResults[$domain] = & $domainHealthScriptBlock -DomainName $domain
        }
        catch {
            Write-Warning "Failed to collect health data for domain '$domain': $($_.Exception.Message)"
            $perDomainResults[$domain] = @()
    ...
    function New-StatusCell {
        ...
        return "<td style='$style$color'>$(ConvertTo-ADOKXmlEscaped ([string]$Value))</td>"
    }
    ...
    function New-MetricCell {
        ...
        return "<td style='${style}background-color:#D9534F;color:#fff;'>$(ConvertTo-ADOKXmlEscaped ([string]$Value))</td>"
    }
...
<p style='font-size:11px;color:#555;margin-top:20px;'>
  Report generated by Get-ADForestHealth &mdash; karanth.ovh
</p>
</body></html>
"@
    ...
    $allDomainTables = foreach ($domain in $allDomains) {
        $table = "<h2 style='color:#174ea6;'>Domain: $(ConvertTo-ADOKXmlEscaped $domain)</h2>"

        if ($domain -in $failedDomains) {
            $table += "<p style='color:#D9534F;font-weight:bold;'>Collection failed for this domain - see console/log output. No data available.</p>"
            $table
            continue
        }
        ...
        foreach ($dc in $perDomainResults[$domain]) {
            $row  = '<tr>'
            $row += "<td style='text-align:center;'><b>$(ConvertTo-ADOKXmlEscaped $dc.Server)</b></td>"
            $row += "<td style='text-align:center;'>$(ConvertTo-ADOKXmlEscaped $dc.Site)</td>"

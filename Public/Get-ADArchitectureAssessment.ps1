function Get-ADArchitectureAssessment {
<#
.SYNOPSIS
...
.DESCRIPTION
    ...
    The function is intentionally read-only. It does not disable services,
    ...
    function to each domain controller. They are not a full vulnerability scan
...
.PARAMETER OutputFolder
    ...
    Defaults to $env:TEMP\ADArchitectureAssessment.
...
.EXAMPLE
    Get-ADArchitectureAssessment -DomainName "corp.contoso.com"

.EXAMPLE
    Get-ADArchitectureAssessment -DomainName "corp.contoso.com" -DomainController "DC01.corp.contoso.com"

.EXAMPLE
    Get-ADArchitectureAssessment -OutputFolder "D:\Reports\ADAssessment" -StaleUserDays 120 -StaleComputerDays 120
    ...
    if (-not $SkipPortChecks) {
        foreach ($dc in $dcInventory) {
            foreach ($port in ($TcpPortsToTest | Sort-Object -Unique)) {
                $portResult = Test-ADOKTcpPortDetail -ComputerName $dc.HostName -Port $port -TimeoutMs $PortTimeoutMs

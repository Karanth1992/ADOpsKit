...
- New public functions go in `Public/`, shared helpers in
  `Private/Helpers.ps1`.

Read an existing function (e.g. [`Get-ADForestHealth.ps1`](Public/Get-ADForestHealth.ps1))
as the canonical reference for structure before writing a new one.

## Tests

- Every public function needs a corresponding file in `Tests/`.
